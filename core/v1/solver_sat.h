...
class PSolverSAT
{
    ...
    {
        int first = rand() % row + 1;
        int second = rand() % column + 1;
        return OrbLocation(first, second);
    }
...
public:
    ...
    std::vector<Route> solve()
    {
        ...
        while (!ok())
        {
            ...
            auto newLoc = OrbLocation(first, second);
            if (!(newLoc == lastLoc) && board.validLocation(newLoc))
            {
                board.swapLocation(currLoc, newLoc);
                lastLoc = currLoc;
                currLoc = newLoc;
            }
        }
